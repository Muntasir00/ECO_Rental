                ...
                <div className="max-w-7xl mx-auto px-4 py-10">
                    ...
                    <div className="mb-8 flex justify-between items-end">
                        <h1 className="text-xl md:text-2xl font-medium text-gray-800">
                            Search Result : <span className="text-gray-500 font-normal">{totalResults} results found</span>
                        </h1>
                        <p className="text-sm text-gray-400">Page {currentPage} of {totalPages || 1}</p>
                    </div>
                    ...
                    {loading && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {[1, 2, 3, 4, 5, 6].map(i => (
                                <div key={i} className="h-[400px] w-full bg-gray-200 animate-pulse rounded-2xl"></div>
                            ))}
                        </div>
                    )}
                    ...
                    {!loading && rooms.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
                            ...
                            <Button variant="link" onClick={() => setSearchParams({})}>Clear Filters</Button>
                        </div>
                    )}
                        ...
                        <div className="mt-16 flex justify-center items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => {
                                    ...
                                    if (p !== 1 && p !== totalPages && (p < currentPage - 1 || p > currentPage + 1)) return null;
                                    ...
                                    return (
                                        <Button
                                            ...
                                            className={`h-10 w-10 rounded-full ${p === currentPage ? 'bg-[#E5555C] hover:bg-[#d4444b]' : ''}`}
                                        >
                                            {p}
                                        </Button>
                                    )
