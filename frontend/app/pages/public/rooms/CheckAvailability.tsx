    ...
    async function onSubmit(data: z.infer<typeof FormSchema>) {
        ...
        try {
            ...
            if (!res.isAvailable) {
                ...
                if (res.totalGuests) {
                    router.push(`/booking?roomId=${id}&start=${formattedStart}&end=${formattedEnd}&availableRooms=${res?.availableRooms}&totalGuests=${res.totalGuests}`);
                } else {
                    router.push(`/booking?roomId=${id}&start=${formattedStart}&end=${formattedEnd}&availableRooms=${res?.availableRooms}`);
                }
            }
        } catch (error) {
            console.log(error)
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                    ...
                    render={({field}) => (
                        <FormItem className="flex flex-col">
                            <FormLabel className="text-xs text-gray-500 mb-1">Check in</FormLabel>
                            <Popover>
                ...
                <FormField
                    ...
                    render={({field}) => (
                        <FormItem className="flex flex-col">
                            <FormLabel className="text-xs text-gray-500 mb-1">Check Out</FormLabel>
                            <Popover>
                                <PopoverTrigger asChild>
                                    <FormControl>
                                        <Button
          ...
          <Button
            type="button"
            className="w-full bg-[#E5555C] hover:bg-[#d4444b] text-white py-6"
          >
            Booking
          </Button>
          */}
