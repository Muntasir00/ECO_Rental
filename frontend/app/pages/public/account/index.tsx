      ...
      <div className='max-w-7xl mx-auto'>
        <div className='flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8'>
          <div>
            ...
            <p className='text-gray-500 font-light'>Welcome {user?.fullName}</p>
          </div>

          <Link to='/profile'>
            <button className='cursor-pointer border border-[#B83E25] text-[#B83E25] hover:bg-red-50 px-8 py-2.5 rounded-sm font-medium transition-colors text-sm'>
              Go to Profile
            </button>
          </Link>
        </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12'>
          {accountOptions.map(option => (
            ...
            >
              {/* Icon */}
              <div className='text-[#1A1A1A]'>{option.icon}</div>
        ...
        ======================== */}
        <div>
          <button className='bg-[#B83E25] text-white hover:bg-[#d4444b] px-8 py-3 rounded-md font-medium shadow-sm transition-colors text-sm'>
