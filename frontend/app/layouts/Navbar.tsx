...
const Navbar = () => {
    ...
    const displayName = profileData?.profile?.fullName || profileData?.user?.username || user?.username || 'User';
    const displayEmail = profileData?.user?.email || user?.email;
    const displayImage = profileData?.profile?.profileImage || 'https://github.com/shadcn.png';

    // Get Initials for Avatar Fallback
    const getInitials = (name: string) => name?.substring(0, 2).toUpperCase() || 'UN';
